...
use super::payloads::{
    ...
    PublicKeyPayload, SecurityPolicyPayload, SignaturePayload,
};
use crate::Error;
...
impl MikeyMessage {
    ...
    pub fn add_timestamp(&mut self, timestamp: u64) {
        ...
        self.payloads
            .push((PayloadType::Timestamp, timestamp.to_be_bytes().to_vec()));
    }
    ...
    pub fn to_bytes_without_mac(&self) -> Vec<u8> {
        ...
        if let Some(header) = &self.common_header {
            ...
            let prf_csp = ((header.prf_func & 0x3F) << 2) | (((header.csp_id >> 8) & 0x03) as u8);
            data.push(prf_csp);
