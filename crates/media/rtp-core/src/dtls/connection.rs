...
impl DtlsConnection {
        ...
        let handle_handshake = async move {
            ...
            impl HandshakeHandler {
                async fn send_record(&mut self, record: Record) -> Result<()> {
                    ...
                    println!(
                        "Sending DTLS record to {}: type={:?}, epoch={}, seq={}, len={}",
                        self.remote_addr,
        ...
        while offset < data.len() {
            ...
            if data.len() - offset < 12 {
                return Err(crate::error::Error::InvalidPacket(format!(
                    "Handshake record too short for header: {} bytes remaining",
                    data.len() - offset
                )));
            }
            ...
            let (header, header_size) =
                super::message::handshake::HandshakeHeader::parse(&data[offset..])?;
            ...
            if data.len() - offset - header_size < header.fragment_length as usize {
                return Err(crate::error::Error::InvalidPacket(format!(
                    "Handshake record too short for message: {} bytes remaining, need {} bytes",
            ...
            let message_data =
                &data[offset + header_size..offset + header_size + header.fragment_length as usize];

            // Parse the message
            let message =
                super::message::handshake::HandshakeMessage::parse(header.msg_type, message_data)?;

            println!(
                "Successfully parsed handshake message: {:?}",
                header.msg_type
            );
            ...
            if let Some(handshake) = self.handshake.as_mut() {
                println!(
                    "Processing handshake message, current state: {:?}",
                    handshake.step()
                );
    ...
    async fn process_change_cipher_spec_record(&mut self, data: &[u8]) -> Result<()> {
        ...
        if data.len() != 1 || data[0] != 1 {
            return Err(crate::error::Error::InvalidPacket(format!(
                "Invalid ChangeCipherSpec message: expected [1], got {:?}",
                data
            )));
        }
        ...
        println!(
            "Updated cipher state, new epoch: {}, sequence number reset to 0",
            self.epoch
        );
    ...
    async fn send_record(&mut self, record: Record) -> Result<()> {
        ...
        let remote_addr = match self.remote_addr {
            ...
            None => {
                return Err(crate::error::Error::InvalidState(
                    "No remote address specified".to_string(),
                ))
            }
        };
            ...
            if let Some(handshake) = &mut self.handshake {
                ...
                while offset < record.data.len() {
                    ...
                    if let Ok((header, header_size)) =
                        super::message::handshake::HandshakeHeader::parse(&record.data[offset..])
                    {
                        // Skip HelloVerifyRequest or Finished (already handled in add_handshake_message)
                        if header.msg_type
                            != super::message::handshake::HandshakeType::HelloVerifyRequest
                            && header.msg_type != super::message::handshake::HandshakeType::Finished
                        {
                            if record.data.len() - offset - header_size
                                >= header.fragment_length as usize
                            {
                                let message_data = &record.data[offset + header_size
                                    ..offset + header_size + header.fragment_length as usize];

                                // Add to transcript
                                println!(
                                    "Adding outgoing message to handshake buffer: {:?}",
                                    header.msg_type
                                );
                                handshake.add_handshake_message(
                                    header.msg_type,
                                    message_data,
                                    is_from_client,
                                );
    ...
    pub async fn continue_handshake(&mut self) -> Result<()> {
        ...
        self.send_handshake_message(HandshakeMessage::ClientHello(client_hello))
            .await?;
        ...
        if let Some(handshake) = self.handshake.as_mut() {
                ...
                && msg_type != super::message::handshake::HandshakeType::Finished
            {
                if let Ok(msg_data) = message.serialize() {
                    ...
                    println!(
                        "Added outgoing message to verification buffer: {:?}",
                        msg_type
                    );
                }
            }
    ...
    pub async fn complete_handshake(&mut self) -> Result<()> {
        ...
        println!("Sending Finished message");
        self.send_handshake_message(HandshakeMessage::Finished(finished))
            .await?;
