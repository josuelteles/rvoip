...
pub async fn connect(
    ...
    start_receive_task: impl Fn(Arc<dyn RtpTransport>) -> Result<(), MediaTransportError>
        + Send
        + 'static,
    ...
    info!(
        "Connecting client to remote address: {}",
        config_remote_address
    );
    ...
    let udp_transport = UdpRtpTransport::new(transport_config).await.map_err(|e| {
        MediaTransportError::ConnectionError(format!("Failed to create UDP transport: {}", e))
    })?;
    ...
    let transport_instance = SecurityRtpTransport::new(Arc::new(udp_transport), is_srtp_enabled)
        .await
        .map_err(|e| {
            MediaTransportError::ConnectionError(format!(
                "Failed to create security transport: {}",
                e
            ))
        })?;
    ...
    *transport_guard = Some(transport_instance.clone());
        ...
        if security_info.mode == SecurityMode::Srtp {
            ...
            if let Some(combined_key) = &srtp_key {
                ...
                if combined_key.len() == 30 {
                    ...
                    debug!(
                        "Creating SrtpCryptoKey with key: {} bytes, salt: {} bytes",
                        key.len(),
                        salt.len()
                    );
                    ...
                    match SrtpContext::new(crypto_suite, crypto_key) {
                        Ok(srtp_context) => {
                            ...
                            if let Some(sec_transport) = transport_instance
                                .as_any()
    ...
    let socket_arc = if let Some(sec_transport) = transport_instance
        .as_any()
        .downcast_ref::<SecurityRtpTransport>()
    {
        ...
        return Err(MediaTransportError::ConnectionError(
            "Failed to get socket handle".to_string(),
        ));
    };
    ...
    if let Some(security) = security {
        // Set remote address
        security
            .set_remote_address(config_remote_address)
            .await
            .map_err(|e| {
        ...
        // Start handshake
        security.start_handshake().await.map_err(|e| {
            MediaTransportError::Security(format!("Failed to start handshake: {}", e))
        })?;
        ...
        if security_requires_dtls {
            ...
            match tokio::time::timeout(handshake_timeout, wait_for_handshake_completion(security))
                .await
            {
                Ok(result) => {
                    result.map_err(|e| {
                        MediaTransportError::Security(format!("Handshake failed: {}", e))
                    })?;
                }
                Err(_) => {
                    return Err(MediaTransportError::Security(format!(
                        "Handshake timed out after {} seconds",
                        security_handshake_timeout_secs
                    )));
                }
            }
        } else {
...
async fn wait_for_handshake_completion(
    security: &Arc<dyn ClientSecurityContext>,
) -> Result<(), MediaTransportError> {
...
pub async fn get_local_address(
    ...
    if let Some(transport) = transport_guard.as_ref() {
        transport.local_rtp_addr().map_err(|e| {
            MediaTransportError::Transport(format!("Failed to get local address: {}", e))
        })
    } else {
        Err(MediaTransportError::Transport(
            "Transport not initialized. Connect first to bind to a port.".to_string(),
        ))
    }
}
...
pub fn is_connected(connected: &Arc<AtomicBool>) -> bool {
