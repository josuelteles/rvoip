...
impl FallbackStats {
    ...
    fn update_success_rate(&mut self) {
        if self.total_packets > 0 {
            self.transcoding_success_rate =
                self.transcoded_packets as f32 / self.total_packets as f32;
        }
    }
    ...
    fn update_avg_latency(&mut self) {
        if self.transcoded_packets > 0 {
            self.avg_transcoding_latency_us =
                self.total_transcoding_time_us / self.transcoded_packets;
        }
    }
...
impl FallbackHandler {
    ...
    pub async fn activate_fallback(
        ...
        info!(
            "🔄 Activating fallback for dialog {}: {} (PT:{}) → {} (PT:{})",
            self.dialog_id,
            detected_codec,
            detected_payload_type,
            self.expected_codec.as_deref().unwrap_or("unknown"),
        ...
        let fallback_mode = if let Some(expected_codec) = self.expected_codec.clone() {
            if self.can_transcode(&detected_codec, &expected_codec) {
                // Attempt transcoding
                match self
                    .setup_transcoding(&detected_codec, &expected_codec, detected_payload_type)
                    .await
                {
                    Ok(()) => FallbackMode::Transcoding {
        ...
        self.stats.transcoding_active =
            matches!(self.current_mode, FallbackMode::Transcoding { .. });
        self.active = true;

        info!(
            "✅ Fallback activated for dialog {}: mode={:?}",
            self.dialog_id, self.current_mode
        );
        Ok(())
    }
    ...
    async fn setup_transcoding(
        ...
        debug!(
            "✅ Transcoding session setup complete for dialog {}",
            self.dialog_id
        );
        Ok(())
    }
    ...
    pub async fn process_frame(&mut self, frame: AudioFrame) -> Result<Option<AudioFrame>> {
        ...
        match &self.current_mode {
            ...
            FallbackMode::Transcoding { .. } => self.process_transcoding(frame, start_time).await,
            FallbackMode::Passthrough { .. } => self.process_passthrough(frame).await,
        }
    }
    ...
    async fn process_transcoding(
        ...
        let transcoder = match self.transcoder.clone() {
            ...
            None => {
                error!(
                    "No transcoding session available for dialog {}",
                    self.dialog_id
                );
                return Ok(None);
            }
        ...
        let transcoding_result = {
            ...
            transcoder_guard
                .transcode(&input_bytes, from_payload_type, to_payload_type)
                .await
        };
        ...
        match transcoding_result {
            Ok(transcoded_bytes) => {
                ...
                if latency_us > self.config.max_transcoding_latency_us {
                    warn!(
                        "High transcoding latency for dialog {}: {}μs",
                        self.dialog_id, latency_us
                    );
                ...
                let transcoded_samples: Vec<i16> = transcoded_bytes
                    .chunks_exact(2)
    ...
    async fn handle_transcoding_error(&mut self) -> Result<()> {
        ...
        if self.error_count >= self.config.max_transcoding_errors {
            warn!(
                "Too many transcoding errors ({}) for dialog {}, switching to passthrough",
    ...
    async fn switch_to_passthrough(&mut self) -> Result<()> {
        if let FallbackMode::Transcoding {
            from_codec,
            from_payload_type,
            ..
        } = &self.current_mode
        {
            info!(
                "🔄 Switching to passthrough mode for dialog {}: {} (PT:{})",
            ...
            info!(
                "✅ Switched to passthrough mode for dialog {}",
                self.dialog_id
            );
        }
...
impl CodecFallbackManager {
    ...
    pub async fn initialize_fallback(
        ...
        debug!(
            "🔧 Initialized fallback for dialog {}: expected codec={:?}, PT={}",
    ...
    pub async fn process_frame(
        ...
        {
            self.handle_detection_result(dialog_id, detection_result)
                .await?;
        }
    ...
    async fn handle_detection_result(
        ...
        match detection_result {
            CodecDetectionResult::UnexpectedCodec {
                ...
                info!(
                    "🔍 Unexpected codec detected for dialog {}: {} (PT:{}, confidence: {:.2})",
                    dialog_id,
                    detected_codec.as_deref().unwrap_or("unknown"),
                    detected_payload_type,
                    confidence
                );
            ...
            CodecDetectionResult::InsufficientData { .. } => {
                ...
                debug!(
                    "Insufficient data for codec detection on dialog {}",
                    dialog_id
                );
            }
        }
...
mod tests {
    ...
    async fn test_fallback_handler_creation() {
        ...
        let handler = FallbackHandler::new(dialog_id.clone(), Some("PCMU".to_string()), 0, config);
    ...
    async fn test_can_transcode() {
        ...
        let handler = FallbackHandler::new(dialog_id, Some("PCMU".to_string()), 0, config);
    ...
    async fn test_fallback_manager_creation() {
        ...
        let dialog_id = DialogId::new("test_dialog");
        let result = manager
            .initialize_fallback(dialog_id.clone(), Some("PCMU".to_string()))
            .await;
        assert!(result.is_ok());
