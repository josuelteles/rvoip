...
impl FromStr for SdpRtpMap {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if encoding_parts.len() < 2 {
            return Err(MediaError::FormatError(
                "Invalid encoding format".to_string(),
            ));
        }

        let encoding_name = encoding_parts[0].to_string();
        let clock_rate = encoding_parts[1]
            .parse::<u32>()
...
impl SdpMediaProcessor {
    ...
    pub fn parse_media_description(
        &self,
        sdp_text: &str,
        ...
        let media_line = media_line
            .ok_or_else(|| MediaError::FormatError("No media line found in SDP".to_string()))?;
    ...
    fn parse_media_line(&self, content: &str) -> Result<SdpMediaLine, MediaError> {
        ...
        if parts.len() < 4 {
            return Err(MediaError::FormatError(
                "Invalid media line format".to_string(),
            ));
        }
        ...
        let payload_types: Result<Vec<u8>, _> =
            parts[3..].iter().map(|pt| pt.parse::<u8>()).collect();
    ...
    pub fn sdp_to_capabilities(
        &self,
        sdp: &SdpMediaDescription,
    ) -> Result<Vec<CodecCapability>, MediaError> {
        ...
        for &payload_type in &sdp.media_line.payload_types {
            let mut codec = if let Some(rtp_map) = sdp.rtp_maps.get(&payload_type) {
                ...
                let mut codec =
                    MediaCodec::new(info.codec_name.clone(), payload_type, info.clock_rate);
    ...
    pub fn capabilities_to_sdp(
        ...
        let direction = if capabilities
            .iter()
            .all(|c| c.direction == MediaDirection::SendOnly)
        {
            MediaDirection::SendOnly
        } else if capabilities
            .iter()
            .all(|c| c.direction == MediaDirection::ReceiveOnly)
        {
            MediaDirection::ReceiveOnly
        } else if capabilities
            .iter()
            .any(|c| c.direction == MediaDirection::Inactive)
        {
    ...
    pub fn negotiate_media_descriptions(
        ...
        self.capabilities_to_sdp(
            &negotiated_caps,
            &local_sdp.media_line.media_type,
            local_sdp.media_line.port,
        )
    }
...
mod tests {
    ...
    fn test_parse_media_line() {
        ...
        let media_line = processor
            .parse_media_line("audio 5004 RTP/AVP 0 8 111")
            .unwrap();
    ...
    fn test_generate_sdp_text() {
        ...
        let sdp = processor
            .capabilities_to_sdp(&[capability], "audio", 5004)
            .unwrap();
