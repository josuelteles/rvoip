    ...
    pub fn negotiate(&self) -> Result<Vec<NegotiationResult>, MediaError> {
        ...
        if compatible_pairs.is_empty() {
            return Err(MediaError::ConfigError(
                "No compatible codecs found".to_string(),
            ));
        }
        ...
        for (local_cap, remote_cap) in compatible_pairs
            .into_iter()
            .take(self.preferences.max_codecs)
        {
        ...
        if results.is_empty() {
            return Err(MediaError::ConfigError(
                "No successful codec negotiations".to_string(),
            ));
        }
