...
/// Test data channel close behavior with RTC sending periodic messages and closing
#[tokio::test]
async fn test_data_channel_close_interop() -> Result<()> {
    ...
    // Track received messages and close events
    ...
    while start_time.elapsed() < test_timeout {
        ...
        // Check if test is complete - WebRTC side should see the close
        ...
        if *webrtc_closed {
            ...
            // Verify WebRTC received all messages before close
            assert!(
                webrtc_msgs.len() >= 3,
