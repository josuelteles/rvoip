...
impl ClientInviteLogic {
    ...
    async fn start_timer_a(
        ...
        let initial_interval_a = timer_handles
            .current_timer_a_interval
        ...
        .await
        {
            Ok(handle) => {
                ...
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), interval=?initial_interval_a, "Started Timer A for Calling state");
            }
            Err(e) => {
                error!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), error=%crate::transaction::safe_diagnostics::SafeOpaqueError::new(&e), "Failed to start Timer A");
            }
        }
    }
    ...
    async fn start_timer_b(
        ...
        .await
        {
            Ok(handle) => {
                ...
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), interval=?interval_b, "Started Timer B for Calling state");
            }
            Err(e) => {
                error!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), error=%crate::transaction::safe_diagnostics::SafeOpaqueError::new(&e), "Failed to start Timer B");
            }
        }
    }
    ...
    async fn start_timer_d(
        ...
        .await
        {
            Ok(handle) => {
                ...
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), interval=?interval_d, "Started Timer D for Completed state");
            }
            Err(e) => {
                error!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), error=%crate::transaction::safe_diagnostics::SafeOpaqueError::new(&e), "Failed to start Timer D");
            }
        }
    }
    ...
    async fn handle_calling_state(
        ...
        debug!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), "ClientInviteLogic: Sending initial request in Calling state");
    ...
    async fn handle_timer_a_trigger(
        ...
        match current_state {
            TransactionState::Calling => {
                debug!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), "Timer A triggered, retransmitting INVITE request");
            ...
            _ => {
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), state=?current_state, "Timer A fired in invalid state, ignoring");
            }
        }
    ...
    async fn handle_timer_b_trigger(
        ...
        match current_state {
            ...
            _ => {
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), state=?current_state, "Timer B fired in invalid state, ignoring");
            }
        }
    ...
    async fn create_and_send_ack_for_response(
        ...
        match utils::create_ack_from_invite(&request_guard, response) {
            ...
            Err(e) => {
                error!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), error=%crate::transaction::safe_diagnostics::SafeOpaqueError::new(&e), "Failed to create ACK for response");
                Err(e)
            }
        }
    }
    ...
    async fn process_response(
        ...
        match current_state {
            TransactionState::Calling => {
                ...
                if is_provisional {
                    // 1xx -> Proceeding
                    common_logic::send_provisional_response_event(tx_id, response, &data.events_tx)
                        .await;
                    ...
                    if let Err(e) = self.create_and_send_ack_for_response(data, &response).await {
                        error!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), error=%crate::transaction::safe_diagnostics::SafeOpaqueError::new(&e), "Failed to ACK failure response");
                        return Ok(Some(TransactionState::Terminated));
                    }

                    common_logic::send_failure_response_event(tx_id, response, &data.events_tx)
                        .await;
            ...
            _ => {
                warn!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), state=?current_state, "Received response in unexpected state");
                return Ok(None);
            }
        }
    }
...
impl TransactionLogic<ClientTransactionData, ClientInviteTimerHandles> for ClientInviteLogic {
    ...
    async fn on_enter_state(
        ...
        match new_state {
            TransactionState::Calling => {
                self.handle_calling_state(data, timer_handles, command_tx)
                    .await?;
            }
            TransactionState::Proceeding => {
                trace!(id=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), "Entered Proceeding state. Timer A discontinued, Timer B continues.");
    ...
    async fn handle_timer(
        ...
        match timer_name {
            "A" => {
                self.handle_timer_a_trigger(data, current_state, timer_handles, self_command_tx)
                    .await
            }
            "B" => {
                self.handle_timer_b_trigger(data, current_state, self_command_tx)
                    .await
            }
            "D" => {
    ...
    pub(crate) fn new_with_route_command_capacity_and_timer_manager(
        ...
        if request.method() != Method::Invite {
            return Err(Error::Other(
                "Request must be INVITE for INVITE client transaction".to_string(),
            ));
        }
...
impl ClientTransaction for ClientInviteTransaction {
    fn initiate(&self) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        ...
        Box::pin(async move {
            tracing::trace!(transaction=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), "ClientInviteTransaction::initiate called");
            ...
            tracing::trace!(transaction=%crate::transaction::safe_diagnostics::SafeTransactionKey::new(&tx_id), "Sending TransitionTo(Calling) command");
            match data
                .cmd_tx
                .send(InternalTransactionCommand::TransitionTo(
                    TransactionState::Calling,
...
mod tests {
    ...
    async fn test_invite_client_initiate_sends_request_and_starts_timers() {
        ...
        assert_eq!(
            ...
            TransactionState::Calling,
            "State should be Calling after initiate"
        );
    ...
    async fn test_invite_client_success_response() {
        ...
        for _ in 0..20 {
            // Give it 20 iterations max for race conditions
    ...
    async fn test_invite_client_failure_response_and_ack() {
        ...
        setup
            .transaction
            .process_response(failure_response.clone())
            .await
            .expect("process_response failed");
        ...
        for _ in 0..20 {
            // More iterations because more events to process
            match TokioTimeout(Duration::from_millis(150), setup.tu_events_rx.recv()).await {
                ...
                Ok(Some(TransactionEvent::StateChanged {
                    ...
                    assert_eq!(transaction_id, *setup.transaction.id());
                    if previous_state == TransactionState::Calling
                ...
                Err(_) => {
                    ...
                    if failure_response_received
                        && calling_to_completed_received
        ...
        assert!(
            failure_response_received,
            "FailureResponse event not received"
        );
        assert!(
            calling_to_completed_received,
            "StateChanged Calling->Completed event not received"
        );
        ...
        assert_eq!(
            setup.transaction.state(),
            TransactionState::Terminated,
            "State should be Terminated after Timer D"
        );
    }
    ...
    async fn test_invite_client_timer_b_timeout() {
        ...
        for _ in 0..30 {
            ...
            match TokioTimeout(Duration::from_millis(1000), setup.tu_events_rx.recv()).await {
                // 1 second timeout // Increased timeout
        ...
        assert_eq!(
            setup.transaction.state(),
            TransactionState::Terminated,
            "State should be Terminated after Timer B"
        );
    }
    ...
    async fn test_invite_client_retransmit_request() {
        ...
        assert!(
            msg_info2.is_some(),
            "Request should have been retransmitted a second time"
        );
    ...
    async fn test_invite_client_retransmit_response_ack() {
        ...
        setup
            ...
            .process_response(failure_response.clone())
            .await
            .expect("process_response for retransmit failed");
        ...
        assert!(
            msg_info.is_some(),
            "ACK should have been sent for retransmitted response"
        );
        if let Some((msg, _)) = msg_info {
            assert!(msg.is_request());
            assert_eq!(msg.method(), Some(Method::Ack));
