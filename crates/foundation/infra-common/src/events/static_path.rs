...
impl StaticFastPathSystem {
    ...
    pub fn new(channel_capacity: usize) -> Self {
        ...
        debug!(
            "Created StaticFastPathSystem with channel capacity {}",
            channel_capacity
        );
    ...
    fn register_event_type<E: Event + 'static>(&self) {
        ...
        if std::any::type_name::<E>().ends_with("::MediaPacketEvent") {
            debug!(
                "Registering MediaPacketEvent with capacity {}",
                self.channel_capacity
            );
        ...
        if !GlobalTypeRegistry::is_static_event::<E>() {
            warn!(
                "Event type {} is not registered as a StaticEvent",
                std::any::type_name::<E>()
            );
        }
    }
...
impl EventSystem for StaticFastPathSystem {
    ...
    fn create_publisher<E: Event + 'static>(&self) -> Box<dyn EventPublisher<E>> {
        ...
        if !self.is_static_event::<E>() {
            warn!(
                "Event type {} is not a StaticEvent, publisher will fail at runtime",
                std::any::type_name::<E>()
            );
        ...
        debug!(
            "Created StaticFastPathPublisher for {}",
            std::any::type_name::<E>()
        );
    ...
    async fn subscribe<E: Event + 'static>(&self) -> EventResult<Box<dyn EventSubscriber<E>>> {
        ...
        // Verify this is a static event
        if !self.is_static_event::<E>() {
            return Err(EventError::InvalidType(format!(
                "Event type {} is not a StaticEvent",
                std::any::type_name::<E>()
            )));
        }
        ...
        debug!(
            "Created StaticFastPathSubscriber for {}",
            std::any::type_name::<E>()
        );
        ...
        F: Fn(&E) -> bool + Send + Sync + 'static,
    {
        ...
        debug!(
            "Created filtered StaticFastPathSubscriber for {}",
            std::any::type_name::<E>()
        );
        ...
        E: Event + 'static,
    {
        ...
        // Create a filtered subscriber directly
        debug!(
            "Created filtered StaticFastPathSubscriber with EventFilter for {}",
            std::any::type_name::<E>()
        );
        Ok(Box::new(FilteredStaticFastPathSubscriber {
            receiver: receiver.inner_receiver().resubscribe(),
...
impl<E: Event + 'static> EventPublisher<E> for StaticFastPathPublisher<E> {
    async fn publish(&self, event: E) -> EventResult<()> {
        ...
        if sender.receiver_count() == 0 {
            ...
            debug!(
                "No active subscribers for {}, skipping publish",
                std::any::type_name::<E>()
            );
            return Ok(());
        }
        ...
        match sender.send(event_arc) {
            ...
            Err(e) => {
                ...
                if sender.receiver_count() == 0 {
                    debug!(
                        "No subscribers for {}, event publishing skipped",
                        std::any::type_name::<E>()
                    );
                    return Ok(());
                }
    ...
    async fn publish_batch(&self, events: Vec<E>) -> EventResult<()> {
        ...
        if sender.receiver_count() == 0 {
            // No subscribers, we can skip sending
            debug!(
                "No active subscribers for {}, skipping batch publish",
                std::any::type_name::<E>()
            );
            return Ok(());
        }
        ...
        for event in events {
            ...
            match sender.send(event_arc) {
                ...
                Err(e) => {
                    ...
                    if sender.receiver_count() == 0 {
                        debug!(
                            "No remaining subscribers for {}, stopping batch publish",
                            std::any::type_name::<E>()
                        );
                        return Ok(());
                    }
...
impl<E: Event + 'static> EventPublisher<E> for InvalidStaticPublisher<E> {
    async fn publish(&self, _event: E) -> EventResult<()> {
        Err(EventError::InvalidType(format!(
            "Event type {} is not a StaticEvent",
            std::any::type_name::<E>()
        )))
    }
...
mod tests {
    ...
    async fn test_static_basic_filtering() {
        ...
        let _subscriber =
            StaticFastPathSubscriber::new(GlobalTypeRegistry::subscribe::<StaticFilterEvent>());
        ...
        let mut filtered_subscriber = FilteredStaticFastPathSubscriber {
            receiver: GlobalTypeRegistry::subscribe::<StaticFilterEvent>()
                .inner_receiver()
                .resubscribe(),
        ...
        for _ in 0..10 {
            // Try more times to ensure we get all events
    ...
    async fn test_static_filtered_subscriber() {
        ...
        for _ in 0..10 {
            // Try up to 10 times (more than needed)
            match filtered_subscriber
        ...
        let unique_ids: Vec<_> = received_ids
            .iter()
            .copied()
        ...
        for id in &received_ids {
            assert!(
                *id > 3,
                "Received event with id <= 3, which should have been filtered out"
            );
        }
    }
